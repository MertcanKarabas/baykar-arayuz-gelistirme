const Waves = () => {
    return (
        <div className="relative">
            <div className="absolute">
                <svg width="1440" height="372" viewBox="0 0 1440 372" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M208 275.94C108.284 265.594 37 289.245 0 327.628V372H1440V1.94758C1397.38 -6.84063 1346.49 16.2356 1326.38 28.8722C1292.55 51.2735 1203.11 94.1807 1115.99 86.5987C1028.87 79.0167 954.268 122.355 927.858 144.971C891.557 175.271 785.577 227.985 652.063 196.451C528 167.149 441.5 212.876 395.823 235.869C358.736 253.819 313.484 286.884 208 275.94Z" fill="#78350F" />
                </svg>
            </div>
            <div className="absolute">
                <svg width="1440" height="564" viewBox="0 0 1440 564" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M208 418.361C108.284 402.675 37 438.532 0 496.727V564H1440V2.95279C1397.38 -10.3713 1346.49 24.6153 1326.38 43.774C1292.55 77.7373 1203.11 142.79 1115.99 131.295C1028.87 119.8 954.268 185.505 927.858 219.795C891.557 265.733 785.577 345.655 652.063 297.845C528 253.42 441.5 322.747 395.823 357.608C358.736 384.822 313.484 434.953 208 418.361Z" fill="#FEF3C7" />
                </svg>
            </div>
            <div className="absolute">
                <svg width="1440" height="684" viewBox="0 0 1440 684" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M208 507.373C108.284 488.351 37 531.837 0 602.413V684H1440V3.58104C1397.38 -12.5779 1346.49 29.8525 1326.38 53.0877C1292.55 94.2772 1203.11 173.171 1115.99 159.23C1028.87 145.289 954.268 224.975 927.858 266.56C891.557 322.272 785.577 419.199 652.063 361.217C528 307.339 441.5 391.416 395.823 433.695C358.736 466.699 313.484 527.496 208 507.373Z" fill="#FDE68A" />
                </svg>
            </div>
        </div>
    )
}

export default Waves;